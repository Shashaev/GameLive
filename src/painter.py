...
class Painter:
    def __init__(
        ...
        self.count_bots = min(count_bots, size_pole)
        ...
        self.lb = tkinter.Label(self.tk, image=self.img_tk)
        self.lb.grid()

        self.id_run_fun = ''

    def render_pole(self, pole: list[list[int]]) -> None:
        self.img_dr.rectangle(
            (0, 0, self.size_pole, self.size_pole),
            fill='white',
        )
    ...
    def starter_fun_in_loop(self, fun: typing.Callable) -> None:
        self.id_run_fun = self.lb.after(
            self.time_loop,
            lambda: self.lb.after_idle(fun),
        )

    def cancel_after(self) -> None:
        self.lb.after_cancel(self.id_run_fun)

    def get_par_win(self) -> tuple[int, int]:
        return self.tk.winfo_rootx(), self.tk.winfo_rooty()

    def run_painter_loop(self) -> None:
        self.lb.mainloop()

    def increase_time_loop(self) -> None:
        self.time_loop *= 2

    def reduce_time_loop(self) -> None:
        if self.time_loop != 1:
            self.time_loop //= 2
